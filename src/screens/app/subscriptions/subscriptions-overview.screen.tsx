...
import { mockSubscriptions } from "./components/mock-subscription";

export const SubscriptionsOverviewScreen = () => {
  const navigate = useNavigate()

  const getMembers = (id: string) => {
    const currentSubscription = mockSubscriptions.reduce((acc, val) => {
      if(id === val._id) {
        acc = val
      }
    ...
    navigate(RoutesConfig.subscriptionMembers, { state: { currentSubscription }})
  }

  return <div className={'container mx-auto space-y-4 sm:p-12 xl:p-16'}>
    {mockSubscriptions.map((item: any, index: number) => {
