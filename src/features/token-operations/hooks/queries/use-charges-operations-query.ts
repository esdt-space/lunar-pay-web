...
export function useChargesOperationsQuery(pageNumber: number, id: string | undefined) {
  const { address} = useGetAccount();

  return useQuery({
    enabled: id !== undefined,
    queryKey: accountChargesOperationsQueryKey(address, id as string),
    queryFn: () => TokenOperationsService.getTokenOperationsByParentId(pageNumber, id as string),
    refetchOnMount: true,
