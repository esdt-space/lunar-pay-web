...
import { singleWalletDonationInteraction } from "@/contracts/lunar-pay/donations/interactions";

export function useSingleWalletDonationMutation() {
  ...
  return useMutation({
    mutationFn: (options: DonationInteractionOptions) => singleWalletDonationInteraction(options),
    onSuccess(sessionId) {
