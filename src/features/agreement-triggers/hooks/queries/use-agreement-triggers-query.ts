...
export function useAgreementTriggersQuery(pageNumber: number, id: string | undefined) {
  const { address} = useGetAccount();

  return useQuery({
    enabled: id !== undefined,
    queryKey: agreementTriggersQueryKey(address, id as string),
    queryFn: () => AgreementTriggersService.getAgreementTriggersById(pageNumber, id as string),
    refetchOnMount: true,
