...
export async function createRecurringAgreementToReceive(token: EsdtToken, amount: number, frequency: string) {
...
const getTransactionData = (token: EsdtToken, amount: number, frequency: string) => {
  ...
  transactionPayload.setFunction(new ContractFunction('createRecuringPaymentAgreementToReceive'))
